...
export default function ServicesShowcase() {
      ...
      () => {
        cards.forEach((card, i) => {
          ...
          /* entrance — content streams in as the panel scrolls into view.
             Use set() + to() rather than from(): from() leaves the later
             staggered items in their NATURAL (visible) state during their
             pre-stagger delay, so on refresh/scroll-in they flash visible,
             snap to hidden, then re-reveal ("glitch and reappear"). set()
             hard-locks the hidden state immediately and keeps it until each
             item's reveal plays. */
        ...
        /* Footer rises over the last stuck card: the trailing spacer keeps the
           last sticky panel pinned while <Footer/> slides up over it. */
        if (footer && spacer) {
  ...
  return (
    <section ref={rootRef} className="relative z-10" aria-label="What we build">
          ...
          >
            ...
            <div className="flex flex-col justify-center gap-5 p-7 sm:p-9 md:p-12 lg:p-14">
              <div data-reveal className="flex items-center gap-4">
                <span className="font-heading text-[clamp(2rem,3.4vw,3.25rem)] font-normal leading-none tabular-nums text-hero-gradient">
                  {s.index}
                </span>
      ...
      {/* Trailing spacer (desktop) — keeps the last card stuck while the footer
          rises over it. */}
      <div ref={spacerRef} className="hidden motion-safe:md:block md:h-[120vh]" aria-hidden />

      {/* Footer — desktop: fixed, slides up over the last stuck card. */}
      ...
      {/* Footer — normal flow on mobile + reduced-motion. */}
